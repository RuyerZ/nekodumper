...
fn get_book(book:u64,conn:&Connection,keys:&HashMap<u64,String>) -> anyhow::Result<String> {
    let mut stmt = conn.prepare("SELECT division_index,division_name FROM division WHERE book_id=? ORDER BY division_index")?;
...
fn get_book_info(book:u64,conn:&Connection) -> anyhow::Result<(String,String)> {
    let mut book_info:Option<String> = conn.query_row(
...
fn main() -> anyhow::Result<()> {
    let keys:HashMap<_,_> = WalkDir::new(KEY_DIR).into_iter().filter_map(|e| e.ok())
        .filter_map(|e| {
    ...
    let books:Vec<_> = WalkDir::new(CPT_DIR).into_iter().filter_map(|e| e.ok())
        .filter_map(|e| {
            let name = e.file_name().to_str()?;
